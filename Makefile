# This Makefile is for the mysql-genocide extension to perl.
#
...
#
#   MakeMaker ARGV: (q[INSTALLDIRS=vendor], q[DEFAULTS], q[QUIET])
#
#   MakeMaker Parameters:

#     AUTHOR => q[Olivier Poitrey <rs@dailymotion.com>]
#     EXE_FILES => [q[mysql-genocide]]
#     NAME => q[mysql-genocide]
#     PREREQ_PM => { Getopt::Long=>q[0], Pod::Usage=>q[0], DBD::mysql=>q[0] }
#     VERSION_FROM => q[mysql-genocide]
...
DISTNAME = mysql-genocide
DISTVNAME = mysql-genocide-0.01
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN1EXT) --perm_rw=$(PERM_RW)\
	  mysql-genocide $(INST_MAN1DIR)/mysql-genocide.$(MAN1EXT) 
...
pure_all :: $(INST_SCRIPT)/mysql-genocide
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) $(INST_SCRIPT)/mysql-genocide

$(INST_SCRIPT)/mysql-genocide: mysql-genocide $(FIRST_MAKEFILE) $(INST_SCRIPT)/.exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/mysql-genocide
	$(CP) mysql-genocide $(INST_SCRIPT)/mysql-genocide
	$(FIXIN) $(INST_SCRIPT)/mysql-genocide
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/mysql-genocide
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         mysql-genocide' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.01' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: mysql-genocide' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  vendor' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    DBD::mysql:                    0' >> META.yml
	$(NOECHO) $(ECHO) '    Getopt::Long:                  0' >> META.yml
	$(NOECHO) $(ECHO) '    Pod::Usage:                    0' >> META.yml
...
$(FIRST_MAKEFILE) : Makefile.PL $(CONFIGDEP)
	...
	$(PERLRUN) Makefile.PL "INSTALLDIRS=vendor" "DEFAULTS" "QUIET"
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>Olivier Poitrey &lt;rs@dailymotion.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi" />' >> $(DISTNAME).ppd
